...
float4 main(VertexOut pin) : sv_target
{
	...
	//float2 temp = clamp(fragend.xy, -1, +1);

	//if (all(temp == fragend.xy))
	//{
	...
	//	return float4(abs(fragend.x) > 1, 0, abs(fragend.y) > 1, 1);
	//}

	//// clip
	//fragend.x = clamp(fragend.x, -1, +1);
	//fragend.y = clamp(fragend.y, -1, +1);

	fragend.xy = fragend.xy * float2(+0.5f, -0.5f) + 0.5f;

	//float2 temp = saturate(fragend.xy);
	...
	//fragend.x = saturate(fragend.x);
	//fragend.y = saturate(fragend.y);
	...
	for (int i = 0; i < steps; ++i)
	{
		...
		if (depth > 0 && depth < thickness * 0.1f)
		{
			hit1 = 1;
			search1 = search0 + ((search1 - search0) / 2);
		}
...
//		1, // step in horizontal or vertical pixels between samples
//		0, // jitter
//		10, // max steps
